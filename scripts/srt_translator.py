...
def srt_to_list(subtitles):
    ...
    def srt_time_to_seconds(srt_time):
        h, m, s_ms = srt_time.replace(",", ":").split(":")
        return int(h) * 3600 + int(m) * 60 + float(f"{s}.{ms}")
    
    return [{"start": srt_time_to_seconds(s["start"]), "end": srt_time_to_seconds(s["end"]), "text": s["text"]} for s in subtitles]
...
def main():
    ...
    translate_srt(args.input_srt, target_lang="fr", refine=not args.no_refine)
